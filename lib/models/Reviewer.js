...
module.exports = class Reviewer {
  ...
  static async insert({ name, company }) {
    const { rows } = await pool.query(
      'INSERT INTO reviewers (name, company) VALUES ($1, $2) RETURNING *', [name, company]
    );
    return new Reviewer(rows[0]);
  }
  ...
  static async remove(id){
    const { rows } = await pool.query(
      `DELETE FROM reviewers WHERE $1 NOT IN
      
      (SELECT reviewers.reviewer_id FROM reviewers
        INNER JOIN reviews
        ON reviewers.reviewer_id = reviews.reviewer_id
        WHERE reviews.reviewer_id = $1)
    ...
    if (!rows[0]) return 'Unable to delete Reviewer';
    return new Reviewer(rows[0]);
  }
};
