...
module.exports = class Film {
  ...
  static async getById(id){
    ...
    let reviews = await pool.query(
      ...
      WHERE reviews.film_id = ($1);`, [id]
    );
    film = film.rows[0];
    cast = cast.rows;
    reviews = reviews.rows;
    // console.log(film, cast, reviews);
    return {
      title: film.title,
      ...
      reviews: reviews.map(review => {
        return { reviewId: review.review_id, rating: review.rating, review: review.review, reviewer: { reviewerId: review.reviewer_id, name: review.name } };
      })
    };
  }
};
